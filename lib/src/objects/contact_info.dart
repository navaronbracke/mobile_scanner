...
class ContactInfo {
  ...
  const ContactInfo({
    ...
    this.name,
    this.organization,
    this.phoneNumbers = const <PhoneNumber>[],
    this.title,
  ...
  factory ContactInfo.fromMap(Map<Object?, Object?>? data) {
    ...
    return ContactInfo(
      addresses: addresses == null
          ? const <Address>[]
          : List.unmodifiable(
              addresses.cast<Map<Object?, Object?>>().map(Address.fromMap),
            ),
      emails: emails == null
          ? const <Email>[]
          : List.unmodifiable(
              emails.cast<Map<Object?, Object?>>().map(Email.fromMap),
            ),
      name: name == null ? null : PersonName.fromMap(name),
      organization: data['organization'] as String?,
      phoneNumbers: phones == null
          ? const <PhoneNumber>[]
          : List.unmodifiable(
              phones.cast<Map<Object?, Object?>>().map(PhoneNumber.fromMap),
            ),
      title: data['title'] as String?,
      urls: urls == null ? const <String>[] : List.unmodifiable(urls.cast<String>()),
    );
  }
